...
DNS exfiltration requires both abnormal DNS upload volume and abnormal
payload/ratio. In this dataset, no such DNS payload anomaly was found. \\
...
HTTPS traffic to countries not observed in training is suspicious. ASN/owner
information is added to support incident analysis. \\
\ruleid{R6} & Anomalous external users &
External clients are flagged by ratio or timing drift. New client IPs are not
automatically anomalous, because the project statement notes that the anomaly
is not traffic amount or flow count. \\
\bottomrule
\end{longtable}
...
No DNS data exfiltration was detected. Although some devices generate many DNS
flows, their DNS payload sizes and upload/download ratios remain compatible
with the clean payload baseline. These anomalies are therefore classified as
DNS C\&C by \ruleid{R4}, not as DNS exfiltration.
...
Low-volume accesses to BE are kept as medium severity because they are unusual
...
explainable and suitable for SIEM reporting. No DNS data exfiltration was
detected; the DNS anomalies are better classified as C\&C due to timing and
flow-count behaviour.
